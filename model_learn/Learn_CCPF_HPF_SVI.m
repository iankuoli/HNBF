...
%
function [] = Learn_CCPF_HPF_SVI(t_i, t_j, tau_i, tau_j, c_param, kappa, matSample_ijv)
    ...
    tmpPhiX = exp(tmpXU(matSample_ijv(:,1),:) + tmpXV(matSample_ijv(:,2),:));
    ...
    nonzero_idx = find(matSample_ijv(:,3)>0);
    E_n_ij = 0.98 * ones(size(matSample_ijv,1), 1);
    range = [0.1:0.1:3];
    phi_n = 1 - c_param + c_param * exp(-range);
    theta_ij = sum(G_matTheta(matSample_ijv(nonzero_idx,1),:) .* G_matBeta(matSample_ijv(nonzero_idx,2),:), 2);
    lambda_ij = log(sum(G_matGamma(matSample_ijv(nonzero_idx,1),:) .* G_matDelta(matSample_ijv(nonzero_idx,2),:), 2)) * range;
    q_prob = -kappa * psi(theta_ij) * phi_n + matSample_ijv(nonzero_idx,3) * log(kappa * phi_n);
    q_prob = exp(bsxfun(@plus, bsxfun(@plus, q_prob, -gammaln(matSample_ijv(nonzero_idx,3)+1)), ...
    ...
    sample_user_idx = find(sum(tmpXX, 2));
    sample_item_idx = find(sum(tmpXX, 1));
    
    for k = 1:K        
        tensorPhiX = sparse(matSample_ijv(:,1), matSample_ijv(:,2), tmpPhiX(:,k) .* E_n_ij, M, N);
        G_matGamma_Shp(sample_user_idx, k) = (1-tau_i) * G_matGamma_Shp(sample_user_idx, k) + ...
                                             tau_i * (a_gamma + sum(tensorPhiX(sample_user_idx,sample_item_idx), 2));
        G_matDelta_Shp(sample_item_idx, k) = (1-tau_j) * G_matDelta_Shp(sample_item_idx, k) + ...
                                             tau_j * (a_delta + sum(tensorPhiX(sample_user_idx,sample_item_idx), 1)');
    end 
